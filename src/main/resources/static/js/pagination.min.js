 ...
 */
(function(){
	console.log("Initiating pagination...");
	$.fn.extend({
		whjPaging:function(b,r){
			var g=$(this);
			if(b==="getPage"){
				return g.get(0)==null||g.get(0).pageText==null?{
					totalSize:0,
					currPage:1,
					pageSize:5,
					totalPage:1
				...
				if(b==="setPage"){
					...
					if(b!=null){
						var o=[{value:5,text:"5条/页",selected:true},
								{value:10,text:"10条/页"},
								{value:15,text:"15条/页"},
								{value:20,text:"20条/页"}
							];
			...
			if(g.get(0).pageText.showPageNum<2){
				...
				if(g.get(0).pageText.totalPage<=g.get(0).pageText.showPageNum){
					...
					}else{if(g.get(0).pageText.currPage-y<1){
						f=1
					}else{
		...
		if(g.get(0).pageText.isShowPageSizeOpt){
			...
			for(var w in g.get(0).pageText.pageSizeOpt){
				...
				t+='<option value="'+g.get(0).pageText.pageSizeOpt[w].value+'" '+j+">"+g.get(0).pageText.pageSizeOpt[w].text+"</option>"}t+="</select>"
			}
			...
			if(g.get(0).pageText.isShowTotalSize){
				...
				t+='<div class="whj_padding whj_color '+a+'">'+l+"</div>"}t+="</div>";
				g.html(t);
				if(g.get(0).pageText.isShowFL){
					if(h=="whj_hover"){
				...
				function u(){
					...
					g.get(0).pageText.callBack(
							g.get(0).pageText.currPage,
							g.get(0).pageText.pageSize
					)
				}
				function c(){
					var k=[];
